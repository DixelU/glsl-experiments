...
vec3 position = vec3(-1.,16.,0.01);//vec3(-7.,-8.,2);
...
float octaves(vec2 uv, vec2 seed, int octaves, bool signed){
	float amp = 0.5;
	float f = 0.;
	for(int i = 1; i < 15;i++)
	{
		if(signed)
			f += (noise(uv, seed) - 0.5)*amp;
		else
			f += noise(uv, seed)*amp;
		uv *= 2.;
		amp *= 0.5;
...
float octaves(vec2 uv, vec2 seed, int octs)
{
	return octaves(uv, seed, octs, false);
}
...
{	
	float combined_z = 0.;
	//areturn 2.;
	//combined_z -= octaves(p.xy * 0.04, vec2(1.,3.), 6, true);
	//combined_z -= octaves(p.yx * 2.1, vec2(7.,1.), 2)*0.01;	
		
	//p.z -= combined_z;
	
	float box1 = sdBox(rotateY(rotateZ(rotateX(p - vec3(-1.1, 18.5, -0.1), 0.3), 0.5), 0.5), vec3(0.1,0.1,0.1));
	float box2 = sdBox(rotateY(rotateZ(rotateX(p - vec3(-1.1, 18.5, -0.1), 0.2), 0.4), 0.6), vec3(0.1,0.1,0.1));
	float box3 = sdBox(rotateY(rotateZ(rotateX(p - vec3(-0.9, 18.4, -0.1), 1.8), 0.5), 0.), vec3(0.1,0.1,0.1));
...
float sc3x(float x) { return pow3(cos(x)) + pow3(sin(x)); }

float northern_lights_in_direction(vec3 p)
{
	float angle = trueAngle(p.x, p.y);
	float height1 = p.z / 0.91 - 1.5 + 
		sc3x(27. * angle + u_time * 0.06) * 0.2 +
		+sc3x(13. * angle + u_time * 0.07) * 0.5 +
		sc3x(2. * angle + u_time * 0.11) * 1.2;
		//sin(27. * angle + u_time * 0.25) * 0.1 + 
		//cos(angle * 21. - u_time * 0.34) * 0.07;
	float puff = octaves(vec2(height1/9., angle*3.), vec2(u_time, 0.)*0.2, 4) /
		max(abs(height1) + 0.1, 0.) * 0.3;
	...
	puff += octaves(vec2(height2/2. + angle/9., angle*3.), vec2(12.*u_time, -u_time * 2.)*0.001, 4) / max(abs(height2) + 0.5, 0.) * 5.5 - 0.3;
	puff += octaves(vec2(-height3/3. - angle/8., angle*4.), vec2(11.*u_time, u_time * 1.7)*0.0095, 4) / max(abs(height2) + 0.5, 0.) * 1.5 - 0.2;
	puff *= octaves(vec2(-height4/8. + angle*4., angle*2.), vec2(7.*u_time, -u_time * 1.9)*0.0032, 4) / max(abs(height4)*0.1 + 0.8, 0.) * 0.5 - 0.1;
...
float normalized_quintic_core(float x, float minval, float maxval, float power)
{
	float middle = (minval + maxval) * 0.5;
	float q = (x - minval) * (x - maxval) / ((middle - minval) * (middle - maxval));
	if(q < 0.)
		return 0.;
	return pow(q, power);
}
...
vec3 sky_color_in_direction(vec3 p){
	...
	if(VCJDECLIPSESIZE >= 0.)
	{
		...
		float puff = octaves(vec2(residual_sun_aligment*2., angle*1.), vec2(u_time, 0.)*0.2, 5);
		puff += octaves(vec2(angle * 2., residual_sun_aligment * 5.), vec2(u_time, 3.)*0.09, 5) * (-0.5);

		if(puff < 0.)
			puff = 0.;
		puff = pow(puff, 2.) * 0.55;
		...
		{			
			...
			else sun = 0.2*exp(-25.*residual_sun_aligment) * (2. + puff * 0.1);
		}
	}
	...
	{ // pyramid
		...
		const float p_z_cutoff_hi = 0.1117;
		const float p_z_cutoff_lo = 0.03;
		const float thickness = 7.;
		const float fadeout_flicker_time = 1e5;
		float fadeout_time = fadeout_flicker_time / (u_time + fadeout_flicker_time);
		
		float leg1 = -abs(dirdiff.z - dirdiff.y + 0.7 * dirdiff.x + pyrasize)*thickness + 0.005;
		float leg2 = -abs(dirdiff.z - dirdiff.y - 0.7 * dirdiff.x + pyrasize)*thickness + 0.004;
		float leg3 = -abs(dirdiff.z - dirdiff.y + 2.4 * dirdiff.x + pyrasize)*thickness + 0.010;

		float legs = max(max(leg1, leg2*0.9), leg3*0.15);
		float legs_flare = clamp(legs * 0.001 + 0.0002, -0., 0.03);
		if(legs_flare > 0.)
		{
			legs_flare *= octaves(p.zx * 5., vec2(3. + 0.01 * u_time, -u_time * 0.051), 3, false);
			float flare_roundoff = normalized_quintic_core(height, p_z_cutoff_hi, p_z_cutoff_lo, 0.5);
			legs_flare *= flare_roundoff;
		}
		
		float outtershell =
			clamp(legs, 0., 0.03) + legs_flare;
		...
		if(SUN_SIZE < 0.)
		{
			...
			float distance_from_pyramid_tip = pow(abs(dirdiff.x), special_dist_p) + pow(abs(height - p_z_cutoff_hi), special_dist_p);
						
			float central_flare = exp(0.25 * flicker / distance_from_pyramid_tip) * 0.01;
			flare += 2. * pow(float(p.z >= p_z_cutoff_hi) * exp(-(p.z - p_z_cutoff_hi)*1.) * (flare_v), 4.);
			//flare += 0.25 * flare_h;
		...
		outtershell *= 
			pow((p.z < p_z_cutoff_hi && p.z > p_z_cutoff_lo) ? 3.* (p.z - p_z_cutoff_lo) * (p_z_cutoff_hi - p.z + 0.01) : 0., 1.0) * fadeout_time;
	...
	{ //damn northern lights
		float angle = trueAngle(p.x, p.y) - PI_C * 0.5;
		float puff = northern_lights_in_direction(p) * exp(-1. * angle * angle);
		float puff2 = northern_lights_in_direction(p + vec3(0.1,0.4,0.025)) * exp(-1. * angle * angle);
		norhtern_lights = 
			vec3(0.01, puff, 0.5) * puff + 
			vec3(0, 0.12, puff2) * puff2 - 0.05;
	}
...
void _mainImage(out vec4 fragColor, in vec2 fragCoord )
{
	...
	direction = rotateY(direction, -0.25*PI_C*(mouse.y - 0.5));
	direction = rotateX(direction, 0.25*PI_C*(mouse.x - 0.5));   
